...
# fail2ban - испульзуем для защиты от атаки методом перебора
# apach2 - веб-сервер, iptables - фаервол, persistent - пакет для сохранения настроек после ребута
...
#Настройка Fail2Ban
#Здесь опять же копируем заранее заготовленные шаблон
# Используется от защиты атаки методом перебора по SSH
...
# Перенос файлов для сайта

rm -rf /var/www/html/index.html
cp assets/apache2/index.html /var/www/html/
cp assets/apache2/img_1.jpg /var/www/html/
cp assets/apache2/img_5.jpg /var/www/html/
cp assets/apache2/nature_small.jpg /var/www/html/
