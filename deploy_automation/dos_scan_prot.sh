...
# -eq - логическое сравнение обозначающие "равны"

if [ "$#" -eq 0 ]
then
	# Set default policy
	# -P флаг установки дефолтных политик
	iptables -P INPUT DROP
	iptables -P OUTPUT DROP
	iptables -P FORWARD DROP
	# Allow traffic on loopback
	# -A для установки правил
	iptables -A INPUT -i lo -j ACCEPT
	iptables -A OUTPUT -o lo -j ACCEPT
	# Allow output connection for icmp
	# -p для указания протокола; -j для указания действия
	# ICMP используется для передачи сообщений об ошибках
	iptables -A INPUT -p icmp -j ACCEPT
	iptables -A OUTPUT -p icmp -j ACCEPT
	# Allow DNS - 53 дефолтный
	# tcp - надежный протокол передачи данных интернета
	# udp - не гарантирует доставку пакетов в нужном виде
	sudo iptables -t filter -A OUTPUT -p tcp --dport 53 -j ACCEPT
	sudo iptables -t filter -A OUTPUT -p udp --dport 53 -j ACCEPT
	sudo iptables -t filter -A INPUT -p tcp --dport 53 -j ACCEPT
	sudo iptables -t filter -A INPUT -p udp --dport 53 -j ACCEPT
	# Allow outgoing http - 80 default
